...
public class HttpMiddleWare {
	...
	// DEPOSITREAMOREPHONEKEY=initIp+"PliesRichesReaMorePhone.do";
	 ...
	 * 短信验证码接口
	 * 
	 * @参数 手机号码：MobilePhone,标示：TokenMessage：,ExistCheckFlag:false,BankId:9999,
	 *     TokenIndex:1
	 * */
	 ...
	 * 
	 * @参数 
	 *     新卡号NewAcNo，交易密码TrsPassword，开户行BankName，手机验证码_pTokenName，1本行或0他行BankInner
	 * */
	 ...
	 * 
	 * @参数 
	 *     原登录密码OldPassword，新登录密码NewPassword，确认登录密码ConfirmPassword，手机动态码_pTokenName
	 * */
	 ...
	 * 交易密码修改：交易密码修改接口
	 * 
	 * @参数 原交易密码TrsPassword，新交易密码NewTrsPassword，确认交易密码ConfirmTrsPassword，
	 *     手机动态码_pTokenName
	 * */
	 ...
	 * 交易密码修改：交易密码重置接口
	 * 
	 * @参数 姓名UserName，证件号码IdNo，证件类型IdType，交易密码NewTrsPassword，
	 *     确认交易密码ConfirmTrsPassword， 手机动态码_pTokenName，Activation=1
	 * */
	...
	public void get(String trade, Map<String, String> map, final boolean isAlowHint, final ResultInterface resultInterface) {
		...
		String params;
		if (trade.equals(Trade_Login)) {
			params = "?v=" + "1.0" + "&format=" + "JSON" + "&method=" + "/oauth2/login";
		} else {
			params = "?v=" + "1.0" + "&format=" + "JSON" + "&method=" + "/oauth2/login" + "&access_token=" + activity.constant.getUserInfo().getAccess_token();
		}
	...
	public void post(String trade, Map<String, String> map, final boolean isAlowHint, final ResultInterface resultInterface) {
		...
		if (map == null) {
			...
			map.put("v", "1.0");
			map.put("format", "JSON");
			map.put("method", "/oauth2/login");
			if (trade.equals(Trade_Login)) {
			} else {
				map.put("access_token", activity.constant.getUserInfo().getAccess_token());
			}
	...
	public void parseResponse(Object response, Boolean isAlowHint, ResultInterface resultInterface) {
		...
		if (repJson.containsKey("errorCode") || JsonUtil.getJSONString(repJson, "success").equals("false")) {
			...
			// errorCode.equals("GA10015"))
			// {// 资金转出特殊处理
			// resultInterface.onError(jsonObject.getString(ErrorKey) +
			// jsonObject.getString(ErrorKeyCode));
			// }
			// else
			// {
			// resultInterface.onError(jsonObject.getString(ErrorKey));
			// }
			// requestErrorHint(activity, isAlowHint, ((JSONObject)
			// errorArray.get(0)).getString(ErrorKey));
			// }
			// else
			// {
			...
			// requestErrorHint(activity, isAlowHint, "通讯错误，请稍后再试！");
			// }
			resultInterface.onError(JsonUtil.getJSONString(repJson, "msg"));
			requestErrorHint(activity, isAlowHint, JsonUtil.getJSONString(repJson, "msg"), JsonUtil.getJSONString(repJson, "errorCode"));
		}
	 ...
	 * 请求失败处理
	 * */
	public void requestErrorHint(final BaseFragmentActivity activity, boolean isAlowHint, String errorMsg, final String errorCode) {
		...
		if (isAlowHint) {
			// AlertUtil.ToastMessageShort(activity.getApplicationContext(),
			// (String) errorMsg);
			AlertUtil.ShowHintDialog(activity, "提示", errorMsg, "确定", true, new OnClickListener() {
				...
				public void onClick(View arg0) {
					// TODO Auto-generated method stub
					if (activity.constant.UserTimeout.equals(errorCode)) {
						activity.constant.setUserInfo(null);
						activity.constant.setLogin(false);
						activity.constant.setToggleString(activity.constant.ACCESS_TOKEN, "");
						activity.constant.setToggleString(activity.constant.EXPIRED, "");
						...
						if (activity instanceof LoginActivity) {
							...
							activity.StartActivity(LoginActivity.class, null);
						}
					}
				}
			});
